...
!!! warning "2026 Update — what changed since this was written (as of May 2026)"
    ...
    - Disaggregated serving (separate prefill and decode pools) is now the mainstream production architecture, not a niche optimization. **NVIDIA Dynamo 1.0 reached GA at GTC, March 2026**, with framework-agnostic KV-cache offloading (GPU → CPU RAM → SSD → network) and LLM-aware routing.
    - Reported gains: up to ~7× throughput on DeepSeek-R1 (Blackwell) and up to ~50× MoE throughput on GB300 NVL72 vs Hopper — driven by disaggregation + FP4 + smarter KV reuse.
    - KV-cache tier offloading via **LMCache** integrated with Dynamo 1.0; SGLang, TensorRT-LLM, and vLLM are all supported backends. Treat prefill/decode disaggregation as a primary design axis when sizing serving infra.

    **Sources:** [NVIDIA — Introducing Dynamo (distributed inference for reasoning models)](https://developer.nvidia.com/blog/introducing-nvidia-dynamo-a-low-latency-distributed-inference-framework-for-scaling-reasoning-ai-models/) · [NVIDIA — Dynamo 1.0 production-ready, multi-node inference](https://developer.nvidia.com/blog/nvidia-dynamo-1-production-ready/) · [LMCache + NVIDIA Dynamo 1.0 (KV-cache offloading)](https://blog.lmcache.ai/en/2026/03/16/lmcache-nvidia-dynamo-1-0-a-match-made-in-inference-heaven/) · [TensorRT-LLM — Disaggregated Serving](https://nvidia.github.io/TensorRT-LLM/blogs/tech_blog/blog5_Disaggregated_Serving_in_TensorRT-LLM.html)
...
!!! info "Official Sources & Further Reading"
    ...
    - [Kwon et al. (2023) — PagedAttention / vLLM paper](https://arxiv.org/abs/2309.06180)
